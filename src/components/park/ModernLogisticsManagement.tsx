...
export default function ModernLogisticsManagement() {
  const [projects, setProjects] = useState<ModernLogistics[]>([
    { id: '1', text: '现代物流产业园招商，打造区域物流枢纽。', address: '园区C区301号', contactPerson: '王经理', phone: '13700137000', email: 'wang@example.com' },
    { id: '2', text: '智慧仓储项目招商，提供智能化解决方案。', address: '园区D区402号', contactPerson: '赵经理', phone: '13612345678', email: 'zhao@example.com' },
  ]);

  const [showAddEditDialog, setShowAddEditDialog] = useState(false);
  const [currentProject, setCurrentProject] = useState<ModernLogistics | null>(null);
  const [formData, setFormData] = useState<Omit<ModernLogistics, 'id'>>({
    text: '',
    address: '',
    contactPerson: '',
    phone: '',
  ...
  const handleAddEdit = () => {
    ...
    if (currentProject) {
      ...
      // Add new project
      const newProject: ModernLogistics = {
  ...
  const handleEditClick = (project: ModernLogistics) => {
  ...
  return (
    <Card className="mt-4">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>现代物流产业招商管理</CardTitle>
        <Button onClick={() => {
          setCurrentProject(null);
          setFormData({ text: '', address: '', contactPerson: '', phone: '', email: '' });
          setShowAddEditDialog(true);
      ...
      <Dialog open={showAddEditDialog} onOpenChange={setShowAddEditDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>填写现代物流产业招商项目的详细信息。</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid_cols-4 items-center gap-4">

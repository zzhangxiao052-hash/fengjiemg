/**
 * Retail Shop Manager Page
 * Display and manage retail shop assets with ProTable
 */
...
const RetailShopPage: React.FC = () => {
  ...
  // Filter retail assets only
  const retailAssets = assets.filter(a => a.type === AssetType.RETAIL);
  ...
  const handleDelete = (asset: Asset) => {
    Modal.confirm({
      title: '确认删除',
      content: `确定要删除门市 "${asset.addressCode}" 吗?`,
      okText: '确认',
      cancelText: '取消',
      onOk: () => {
  ...
  return (
    <div>
      <UnifiedStatisticsCard stats={stats} title="商铺资产统计" />
      <ProTable<Asset>
        columns={columns}
        actionRef={actionRef}
      ...
      >
        <ProFormText
          ...
          label="门市编码"
          placeholder="例: S1-1"
          rules={[{ required: true, message: '请输入门市编码' }]}
        />
